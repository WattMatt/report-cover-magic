...
function PageTemplateManager<T>({
  ...
  const { templates, loading, saveTemplate, deleteTemplate } = useCloudPageTemplates<T>(pageType);
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    await saveTemplate(templateName.trim(), currentData, primaryLineColor, accentLineColor);
    setIsSaving(false);
    setTemplateName("");
